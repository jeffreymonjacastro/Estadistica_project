...
```{r}
encuesta %>% group_by(TotalMentalHealth) %>% count() -> probMentalHealth
probMentalHealth %>% rename(Frecuencia = n) -> probMentalHealth
probMentalHealth$ProbabilidadAtomica <- probMentalHealth$Frecuencia / nrow(encuesta)
probMentalHealth
```
