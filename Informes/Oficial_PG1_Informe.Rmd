...
encuesta <- encuesta[encuesta$Salary < 150000,]
```
...
encuesta$Anciedad <- columna_booleana
...
ansiedad <- filter(encuesta, Anciedad == TRUE)
...
```{r}
encuesta %>%
 filter(Salary >= 3611 & Salary <= 150000) %>%
 ggplot() +
  aes(x = Salary, y = Memoria) +
...
```{r}
encuesta %>%
 filter(Salary >= 3611L & Salary <= 150000) %>%
 ggplot() +
  aes(x = Salary, y = Emocional) +
...
```{r}
tabla2 <- data.frame(Mental_Health = c("Memoria","Emocional", "Anciedad", "Autismo", "Aprendizaje"),
                    Total = c(sum(encuesta$Memoria== TRUE),
                                  sum(encuesta$Emocional == TRUE),
                                  sum(encuesta$Anciedad == TRUE),
...
TotalAnciedad <- sum(encuesta$BSD == TRUE & encuesta$Ansiedad == TRUE),
...
TotalAnciedad = sum(encuesta$Linux_Based == TRUE & encuesta$Anciedad == TRUE),
...
TotalAnciedad = sum(encuesta$Windows == TRUE & encuesta$Anciedad == TRUE),
...
TotalAnciedad = sum(encuesta$Windows_for_Linux == TRUE & encuesta$Anciedad == TRUE),
...
TotalAnciedad = sum(encuesta$Mac == TRUE & encuesta$Anciedad == TRUE),
...
# Calcular el total de Usuarios por cada sistema operativo
