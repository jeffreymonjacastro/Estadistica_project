...
Veamos cuántas filas y columnas tiene nuestro `languages`:
...
Como se puede ver, hay muchas columnas (en total 356); sin embargo no utilizaremos todas para este estudio.

Por ello, procedemos a limpiar nuestro `languages`, centrándonos en las variables que nos interesan, las cuales son: `Title`, `Appeared`, `Type`, `Rank`, `numberOfJobs`, `numberOfUsers`, `Country`, `githubLanguage.repos` y `githubBigQuery.repos`.

Vamos a eliminar las columnas que no necesitemos:

```{r}
# Columnas a conservar
languages <- select(languages, title, appeared, type, rank, numberOfJobsEstimate, numberOfUsersEstimate, country, githubLanguage_repos, githubBigQuery_repos)
```
...
```{r}
languages %>% rename(Nombre = title,
                     Año = appeared,
                     Tipo = type,
                     Rank = rank,
                     N_Trabajos = numberOfJobsEstimate,
                     N_Usuarios = numberOfUsersEstimate,
                     País = country,
                     Github_repos = githubLanguage_repos,
                     BigQuery_repos = githubBigQuery_repos) -> languages
```
...
```{r}
languages$Año[languages$Año < 1945] <- NA

#languages <- languages[!is.na(languages$Año), ]
```
...
```{r}
languages <- languages[languages$Tipo == "pl" | languages$Tipo == "queryLanguage" | languages$Tipo == "textMarkup" | languages$Tipo == "stylesheetLanguage" ,]
```
...
```{r}
languages$Rank <- languages$Rank + 1
```
...
```{r}
languages$N_Trabajos[languages$N_Trabajos == 0] <- NA
```
...
```{r}
languages <- languages[languages$N_Usuarios > 10,] 
```
...
```{r}
languages$Github_repos[languages$Github_repos == 0] <- NA
```
...
```{r}
languages$BigQuery_repos[languages$BigQuery_repos == 0] <- NA
```
...
for (i in 1:nrow(languages)){
  if (languages$Nombre[i] %in% language){
    fila <- subset(languages, Nombre == languages$Nombre[i])
    df <- rbind(df, fila)
  }
}
