...
for (i in 1:nrow(planguages)){
  if (planguages$Nombre[i] %in% language){
...
```{r}
language %>% arrange(desc(N_Usuarios)) -> OrdenUsuarios
...
```{r}
language %>% arrange(desc(N_Trabajos)) -> OrdenTrabajos
...
```{r}
language %>% arrange(desc(Github_repos)) -> OrdenGithubrep
...
```{r}
language %>% arrange(desc(BigQuery_repos)) -> OrdenBigQueryrep
...
```{r}
encuesta %>%
 filter(Salary >= 3611 & Salary <= 758445) %>%
 ggplot() +
...
```{r}
encuesta %>%
 filter(Salary >= 3611L & Salary <= 758445L) %>%
 ggplot() +
  aes(x = Salary, y = Emotional) +
...
```{r}
encuesta %>%
 filter(Salary >= 3611L & Salary <= 255222L) %>%
 ggplot() +
  aes(x = Salary, y = Learning) +
  geom_boxplot(fill = "#FFD6F8") +
  theme_minimal()
```
