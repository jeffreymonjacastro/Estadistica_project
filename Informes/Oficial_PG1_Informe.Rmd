...
| MentalHealth          | Categórica nominal | Como considera que está su salud mental                    | Se omiten "*In your own words*" y "*Prefer not say*" |
| ProfessionalOP        | Categórica nominal | Sistema operativo que usan para trabajar profesionalmente  | \-                                                   |
| ProgrLanguages        | Categórica nominal | Lista de los lenguajes que han usado                       | \-                                                   |
...
OP <- function(SisOp){
  ...
  data.frame(ProfessionalOS = deparse(SisOp),
             TotalMemoria = sum(encuesta[[SisOp]] == TRUE & encuesta$Memoria == TRUE),
             TotalEmocional = sum(encuesta[[SisOp]] == TRUE & encuesta$Emocional == TRUE),
             TotalAnsiedad = sum(encuesta[[SisOp]] == TRUE & encuesta$Ansiedad == TRUE),
             TotalAutismo = sum(encuesta[[SisOp]] == TRUE & encuesta$Autismo == TRUE),
             TotalAprendizaje = sum(encuesta[[SisOp]] == TRUE & encuesta$Aprendizaje == TRUE),
...
total_cant_users <- aggregate(Total ~ ProfessionalOS, 
                              data = tabla_final, 
                              FUN = sum)
...
tabla_final_completa <- tabla_final_completa[order(tabla_final_completa$ProfessionalOS), ]

ggplot(tabla_final_completa, 
       aes(x = ProfessionalOS, fill = MentalHealth)) +
       geom_bar(aes(y = Total), 
                stat = "identity", 
                position = "stack") +
       geom_bar(aes(x = ProfessionalOS, y = Total), 
                stat = "identity", 
                fill = "gray", 
                width = 0.3) +
       geom_text(aes(x = ProfessionalOS, y = Total, label = Total), 
                 vjust = -0.5, 
                 color = "black", 
                 size = 3, 
                 hjust = 1) +
